#!/usr/bin/env python3
"""
Final Metformin Historical Data Generator for Sales Rep "ace" (2000-2025)
"""
...
def generate_metformin_final():
    print("=== Final Metformin Data Generator for Sales Rep 'ace' ===")
        ...
        if daily_sales > 0 and current_stock >= daily_sales:
            try:
                ...
                cursor.execute("""
                    ...
                    (id, order_number, sales_rep_id, customer_name, customer_phone, customer_address,
                ...
                """, (
                    order_id,
                    f"MET-{current_date.strftime('%Y%m%d')}{order_id:04d}",
                    ace_sales_rep_id,
                    ...
                    round(float(metformin.unit_price * daily_sales), 2),
                    'delivery',
                    f"Delivery Address {order_id}",
                    "Standard delivery by ace",
                    False,  # prescription_required
                    True,   # prescription_verified
                    f"Customer notes for order {order_id}",
                    f"Order created by ace - Metformin {current_date.strftime('%Y-%m-%d')}",
                ...
                cursor.execute("""
                    ...
                    (id, order_id, medicine_id, quantity, unit_price, total_price, prescription_notes, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    order_id,
                    order_id,
                    ...
                    round(float(metformin.unit_price * daily_sales), 2),
                    '',  # prescription_notes (empty string for non-prescription orders)
                    current_date.isoformat()
                ))
